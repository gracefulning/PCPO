...
from omnisafe.common.buffer.pcpo_buffer import PcpoBuffer
from omnisafe.typing import DEVICE_CPU, AdvatageEstimator, OmnisafeSpace
from omnisafe.utils import distributed
from collections import defaultdict


class PcpoVectorBuffer(PcpoBuffer):
    ...
    def finish_path_cost(
        ...
        """
        self.buffers[idx].finish_path_cost(last_value_r, last_value_c)
        
    def finish_path_reward(
        self,
    ) -> None:
        """Get the data in the buffer.
        ...
        for buffer in self.buffers:
            if len(adv_c) == 0:
                adv_c = buffer.data['adv_c']
            else:
                adv_c = torch.cat([adv_c, buffer.data['adv_c']], dim=0)   
            for reward in buffer.rewards:
                if len(r) == 0:
                    r = reward[:-1]
                else:
                    r = torch.cat([r, reward[:-1]], dim=0)
        adv_c_mean = torch.mean(adv_c)
        adv_c_std = torch.std(adv_c)
        reward_max = torch.max(r) 

        int_r = []
        for buffer in self.buffers:
            for idx, (start, end) in enumerate(zip(buffer.path_start_idx_list, buffer.path_end_idx_list)):
                buffer.finish_path_extra_reward(idx, start, end, adv_c_mean, adv_c_std)
            for I in buffer.I:
                if len(int_r) == 0:
                    int_r = I
                else:
                    int_r = torch.cat([int_r, I], dim=0)

        I_mu = torch.mean(int_r)
        I_std = torch.std(int_r)
        I_max = torch.max(int_r)

        for buffer in self.buffers:
            for idx, (start, end) in enumerate(zip(buffer.path_start_idx_list, buffer.path_end_idx_list)):
                buffer.finish_path_reward(idx, start, end, reward_max, I_mu, I_std, I_max)
