...
class PcpoBuffer(BaseBuffer):  # pylint: disable=too-many-instance-attributes
    ...
    def finish_path_cost(
        ...
        if self.path_start_idx == 0:
            self.rewards = []
            self.costs = []
            self.values_r = []
            self.path_start_idx_list = []
            self.path_end_idx_list = []

        self.rewards.append(rewards)
        self.costs.append(costs)
        self.values_r.append(values_r)
        self.path_start_idx_list.append(self.path_start_idx)
        self.path_end_idx_list.append(self.ptr)
        
        self.path_start_idx = self.ptr
    
    def finish_path_extra_reward(self, idx, start, end, adv_c_mean, adv_c_std):
        """This function is used to calculate the extra reward for the current path, which is used to
        adjust the reward of the current path based on the cost.

        Args:
            idx (int): The index of the buffer.
            start (int): The start index of the path.
            end (int): The end index of the path.
            reward_mean (float): The mean of the reward.
            reward_std (float): The standard deviation of the reward.
            adv_c_mean (float): The mean of the advantage of the cost.
            adv_c_std (float): The standard deviation of the advantage of the cost.

        Returns:
            dict: A dictionary containing the extra information for logging.
        """
        path_slice = slice(start, end)
        adv_c = self.data['adv_c'][path_slice]
        costs = self.costs[idx]
        
        mask = (adv_c < 0).float()
        adv_c_norm = (adv_c - adv_c_mean) / (adv_c_std + 1e-6) if adv_c.shape[0] > 1 else adv_c
        slack = abs(-((costs.sum() - self._cost_limit))).clamp(min=1e-4)
        I = mask * abs(adv_c_norm/((1-self._gamma)*slack))
    ...
    def finish_path_reward(self, idx, start, end, reward_max, I_mu, I_std, I_max):
        path_slice = slice(start, end)

        I = (self.I[idx]- I_mu) / (I_std + 1e-6) if self.I[idx].shape[0] > 1 else self.I[idx]
        I = torch.softmax(I/self._beta, dim=0)
        I = torch.sigmoid((self._cost_limit * self._alpha - self._alpha2 * abs(self.slack[idx]))) * I

        rewards = self.rewards[idx]
        values_r = self.values_r[idx]
        
        eta = (self._omega * reward_max) / (I_max + 1e-6) 
    
        rewards[:-1] = rewards[:-1] + eta * I 
