...
from omnisafe.common.buffer import PcpoVectorBuffer
...
class PolicyGradient(BaseAlgo):
    ...
    def _init(self) -> None:
        ...
        """
        self._buf: PcpoVectorBuffer = PcpoVectorBuffer(
            obs_space=self._env.observation_space,
            act_space=self._env.action_space,
            ...
            device=self._device,
            alpha=self._cfgs.algo_cfgs.alpha if self._cfgs.algo_cfgs.get('alpha', None) is not None else 1.,
            alpha2=self._cfgs.algo_cfgs.alpha2 if self._cfgs.algo_cfgs.get('alpha2', None) is not None else 1.,
            beta=self._cfgs.algo_cfgs.beta if self._cfgs.algo_cfgs.get('beta', None) is not None else 1.,
            omega=self._cfgs.algo_cfgs.omega if self._cfgs.algo_cfgs.get('omega', None) is not None else 0.1,
            cost_limit= self._cfgs.algo_cfgs.cost_limit if self._cfgs.algo_cfgs.get('cost_limit') is not None else self._cfgs.lagrange_cfgs.cost_limit,
        )
    ...
    def _init_log(self) -> None:
        ...
        if self._cfgs.model_cfgs.actor_type == 'gaussian_learning' or self._cfgs.model_cfgs.actor_type == 'mec_actor':
        ...
        self._logger.register_key('Value/Adv_c_max')
        self._logger.register_key('Value/Adv_c_min')
